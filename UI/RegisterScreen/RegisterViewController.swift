    ...
    @IBAction func registerBtnTapped(_ sender: UIButton) {
        ...
        if passwordTextField.text != rewritePassTextField.text {
            ...
            if (!ValidationHelpers().isValidEmail(testStr: emailTextField.text!)){
                ...
                let param = ["name": nameTextField.text! , "email":emailTextField.text!, "password":passwordTextField.text!,"phoneNumber": phoneTextField.text!] as [String: Any]
                
                let targetUrl = "http://mesawer.getsandbox.com/user"
                let serviceClient = AlamaofireClient()
                serviceClient.requestPOSTURL(url: targetUrl, param: param, success: { (success) in
                    print("Registeration Done")
                    
                    self.performSegue(withIdentifier:"showCatlog", sender: self)
                    
                    let def = UserDefaults.standard
                    def.set(self.emailTextField.text, forKey: "email")
                    def.set(self.passwordTextField.text, forKey: "password")
                }) { (failure) in
